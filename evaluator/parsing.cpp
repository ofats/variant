...
calc_node f_nonterm(prs::input_data& data) {
    if ('(' == peek(data)) {
        ...
        data >>= prs::next_if<')'>() >> prs::skip_spaces();
        return result;
    }

    if ('s' == peek(data)) {
        data >>= prs::next() >> prs::next_if("in(") >> prs::skip_spaces();
    ...
    if ('c' == peek(data)) {
        data >>= prs::next() >> prs::next_if("os(") >> prs::skip_spaces();
    ...
    if ('l' == peek(data)) {
        data >>= prs::next() >> prs::next_if("og(") >> prs::skip_spaces();
